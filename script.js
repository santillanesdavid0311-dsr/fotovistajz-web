...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Botón de WhatsApp del anuncio "Para fotógrafos"
  const photogWhatsappBtn = document.getElementById('photogWhatsappBtn');
  if (photogWhatsappBtn) {
    const msg = encodeURIComponent('Hola, soy fotógrafo y quiero imprimir mi trabajo con ustedes. ¿Me pueden dar información de precios y del descuento de fotógrafo?');
...
/* ============================================================
   PAQUETES
   Todavía no hay combos ni precios — por eso no tiene pasos de
   elegir/confirmar como los demás, solo manda una pregunta directa.
   En cuanto la jefa tenga los datos, se arma igual que Cuadro o Póstumas.
   ============================================================ */
function startPaquetes() {
  showView('view-paquetes');
}
function submitPaquetes(channel) {
  const msg = 'Hola, quiero preguntar por los paquetes/combos de Fotovista. ¿Me pueden dar información?';
  if (channel === 'whatsapp') {
    window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`, '_blank');
  } else {
    const subject = encodeURIComponent('Pregunta — Paquetes Fotovista');
